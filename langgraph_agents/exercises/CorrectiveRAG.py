...
class GradeDocuments(BaseModel):
    ...
    binary_score: Literal["yes", "no"] = Field(
        ...,
        description="Are the retrieved documents relevant to the user's question? Answer 'yes' or 'no'."
    )
...
def router_node(state: AgentState) -> AgentState:
    """
    An LLM node that evaluates the retrieved documents for relevance against the query.
    """
    # FIX: Updated system prompt to match the binary grading schema
    system_prompt = """You are an expert data auditor grading document relevance for a Land Investment System.
    Analyze the provided context chunks carefully and determine if they contain useful information to help answer the user's question.
    Provide a strict binary score: return 'yes' if the context is highly relevant and useful, or 'no' if it is irrelevant, outdated, or lacks the necessary facts to answer the question."""
    ...
    state["route"] = result.binary_score
    return state
...
def decide_next_node(state: AgentState) -> str:
    ...
    if decision == "no":
        ...
        return "generate_response" # The name of your Phase 1 vector search node  
...
graph.set_entry_point("retrieve_docs_node")

graph.add_edge("retrieve_docs_node", "router")
...
if __name__ == "__main__":
    ...
    user_query = "What type of questions should i practice for agentrix hackathon?"
